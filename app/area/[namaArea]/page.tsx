...
async function fetchWaktu(lat: string, lon: string): Promise<WaktuData> {
    const response = await fetch(
      `https://api.timezonedb.com/v2.1/get-time-zone?key=EYCRQZN0DGJ8&format=json&by=position&lat=${lat}&lng=${lon}`,
      { cache: 'no-store' }
    );
    if (!response.ok) {
        throw new Error(`Timezonedb API error: ${response.status} ${response.statusText}`)
    }
    const waktuData: WaktuData = await response.json()
    return waktuData
}
